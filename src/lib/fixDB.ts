...
export default async (knex: Knex): Promise<void> => {
  ...
  const asoVisionExists = await db("o_agentDeploy").where("key", "asoVisionAi").first();
  if (!asoVisionExists) {
    await db("o_agentDeploy").insert({
      model: "",
      modelName: "",
      ...
      key: "asoVisionAi",
      name: "ASO 图片理解",
      desc: "用于 ASO 创意方案生成时理解参考图片（Vision），建议使用支持 image input 的多模态模型，如 GPT-4o、Gemini Vision",
      disabled: false,
    });
  }
  //矫正提示词
